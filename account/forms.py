...
# SET USER TYPE
from lacaravane.models import UserProfile
...
class LoginForm(forms.Form):
    ...
    def clean_login(self):
        login = self.cleaned_data['login']
        return login.strip()
#Start copy signup process for forum user 
    def clean(self):
        if self._errors:
            return
        ...
        #custom check
        if user:
            userprofile = UserProfile.objects.get_or_none(user_id=user.id)

            if userprofile.forumpassword:
                pass
            else:   
                credentials = {}
                login = self.cleaned_data["login"]
                ...
                url = settings.SITE_URL+'/forum/django_login.php'
                r = requests.post(url, params=details)
                data = json.loads(r.text)
                if data['status']==1: 
                    userprofile.forumpassword = password
                    userprofile.save()
            self.user = user
            #default
            if user.is_active:
            ...
            if data['status']==1:
                try:
                    exuse = get_user_model().objects.get(username = username)
                except:
                    exuse=False
                ...
                if(exuse):
                    ...
                    user = get_user_model().objects.create(username = username, email=data['user_email'], is_active=True)
                    user.set_password(password)
                    user.save()
                    if not password:
                        password = User.objects.make_random_password()
                    userprofile = UserProfile.objects.create(user_id=user.id, forumpassword=password)
                    userprofile.save()
                    self.user = user
            else:  
                raise forms.ValidationError(
...
class BaseSignupForm(_base_signup_form_class()):
    ...
    def clean_email(self):
        ...
        cursor.execute("""SELECT user_email FROM bb3_users WHERE user_email='%s'""" %(str(value)))
        row = cursor.fetchone ()
        cursor.close ()    
...
class SignupForm(BaseSignupForm):
    ...
    def clean(self):
        ...
        if app_settings.SIGNUP_PASSWORD_VERIFICATION \
            ...
            values = {
                'k': settings.RECAPTCHA_PUBLIC_KEY
            }
            ...
            if result['success']:
                ...
                raise forms.ValidationError(_("reCaptcha response from Google not valid, try again."))
        return self.cleaned_data
...
class ResetPasswordForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if not self.users.exists():
            details = {'email': email}
            url = settings.SITE_URL+'/forum/django_forgot.php'
            #url = 'http://localhost:8888/demo/text.php'
            r = requests.post(url, params=details)
            data = json.loads(r.text)
            if data['status']==1:
                user = get_user_model().objects.create(username = data['username'], email=email, is_active=True)
                user.save()
